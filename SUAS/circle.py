...
def detect_triangles(image_path):
    ...
    # Print the percentage of recognized triangles
    total_triangles = len(triangles)
    print(f"Total Triangles: {total_triangles}")

    for idx, triangle in enumerate(triangles, 1):
        # Convert color to float32 for area calculation
        color_float32 = color.astype(np.float32)
        area_percentage = cv2.contourArea(triangle) / cv2.contourArea(color_float32)
        print(f"Triangle {idx}: {area_percentage:.2%}")
